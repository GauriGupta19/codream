...
class MoonClient(BaseClient):
    ...
    def local_train(self):
        ...
        """
        avg_loss = self.model_utils.train_fedcon(self.model, self.global_model, self.prev_models, 
                                                  self.cos, self.mu, self.temperature, self.optim,
    ...
    def set_previous_models(self):
        ...
        if len(self.prev_models) < self.model_buffer_size:  
            self.prev_models.append(old_model)
        elif self.pool_option == 'FIFO':
            for i in range(self.model_buffer_size-2, -1, -1):
                self.prev_models[i] = self.prev_models[i+1]
            self.prev_models[self.model_buffer_size - 1] = old_model
