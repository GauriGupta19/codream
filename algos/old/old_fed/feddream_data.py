...
class FedDreamClient(BaseClient):
    ...
    def single_round_fast(self):
        ...
        for g_step in range(self.g_steps):
            ...
            if self.ismaml:
                if g_step==0: self.meta_optimizer.zero_grad()
                fomaml_grad(self.meta_generator, self.generator, self.device)
                if g_step == (self.g_steps-1): self.meta_optimizer.step()
        ...
        reps = self.comm_utils.wait_for_signal(src=self.server_node,
                                                tag=self.tag.REPS_DONE) 
        reps = self.generator(reps.to(self.device))
        self.comm_utils.send_signal(dest=self.server_node,
                                        data=reps.to("cpu"),
                                        tag=self.tag.REPS_DONE)
    ...
    def run_protocol(self):
        ...
        self.s_model = self.model_utils.get_model(self.config["model"], self.config["dset"], 
...
class FedDreamServer(BaseServer):
    ...
    def single_round_fast(self):
        ...
        for client in self.clients:
            self.comm_utils.send_signal(dest=client, data=(self.reps), 
                                            tag=self.tag.REPS_DONE)
        reps = self.comm_utils.wait_for_all_clients(self.clients, tag=self.tag.REPS_DONE)
        reps = torch.stack(reps).to(self.device)
        self.reps = reps.mean(dim=0)
        return
