...
class FedDreamFastClient(BaseClient):
    ...
    def set_algo_params(self):
        ...
        self.nz = 256
        self.model2 = self.model_utils.get_model(self.config["model"], self.config["dset"], self.device, self.device_ids, num_classes=10)
    ...
    def update_local_model(self):
        ...
        for epoch in range(self.config["distill_epochs"]):
            ...
            student_loss, student_acc = self.model_utils.train(self.model2,
                                                                self.optim,
                                                                synth_dloader,
                                                                self.kl_loss_fn,
                                                                self.device,
            ...
            test_loss, test_acc = self.model_utils.test(self.model2,
                                                        self._test_loader,
                                                        self.loss_fn,
                                                        self.device)
...
class FedDreamFastServer(BaseServer):
    ...
    def update_stats(self, round, stats: List[List[float]]):
        """
    ...
    def run_protocol(self):
        ...
        for client in self.clients:
            ...
            test_acc = self.comm_utils.wait_for_all_clients(self.clients, tag=self.tag.CLIENT_STATS)
            self.log_utils.log_console("Test acc after warmp, Teacher acc {}".format(test_acc))
        ...
        for round in range(start_epochs, total_epochs):
            ...
            if self.log_console:
                self.log_utils.log_console("Round {} done".format(round))
        for i in range(self.config["distill_epochs"]):
            stats = self.comm_utils.wait_for_all_clients(self.clients, tag=self.tag.CLIENT_STATS)
            self.update_stats(i, stats)
            self.log_utils.log_console("Distill Round {} done".format(i))
