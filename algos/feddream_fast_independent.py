...
class FedDreamFastClientIndp(BaseClient):
    ...
    def add_dreams(self):
        ...
        out = self.model(self.reps).detach().to("cpu")
    ...
    def single_round_fast(self):
        ...
        for g_step in range(self.global_steps):
            ...
            self.reps = self.generator(grads)   
        if self.bn_mmt != 0:
            for h in self.hooks:
                h.update_mmt()  
        self.add_dreams()
...
class FedDreamFastServerIndp(BaseServer):
    """
    ...
    def add_dreams(self):
        reps = self.comm_utils.wait_for_all_clients(self.clients, tag=self.tag.FINAL_REPS)
        ...
        if self.adaptive_distill:
            for i in range(len(reps)):
                self.dset.append((reps[i], acts[i]))
        # if self.log_console:
        #         self.log_utils.log_tensor_to_disk((reps, acts), f"node", self.round)
